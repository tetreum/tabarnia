...
namespace Peque.Traffic
{
    public class WaypointNavigator : MonoBehaviour
    {
        ...
        public bool isBranching {
            get {
                if (previousWaypoint != null && previousWaypoint.branches.Count > 0) {
                    foreach (WaypointData point in previousWaypoint.branches) {
                        if (destination == point.centerPosition || currentWaypoint == point) {
                            return true;
                        }
                    }
                }
        ...
        private void OnDestroy() {
            if (currentWaypoint != null) {
                currentWaypoint.occupied = false; // release the waypoint
            }
        }
